...
func ThriftRunStatusToDomain(thrift *worker.RunStatus) runner.RunStatus {
	...
	domain.BazelResult = bazelapi.MakeActionResultDomainFromThrift(thrift.BazelResult_)
	return domain
}
...
func DomainRunStatusToThrift(domain runner.RunStatus) *worker.RunStatus {
	...
	thrift.BazelResult_ = bazelapi.MakeActionResultThriftFromDomain(domain.BazelResult)
	return thrift
}
