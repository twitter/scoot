...
func StartServer(
	...
	memCap uint64,
	memLeakThreshold uint64,
	stat *stats.StatsReceiver,
) {
	// create worker object:
	// worker support objects
	memory := execer.Memory(memCap)
	memLT := execer.MemoryLeakThreshold(memLeakThreshold)
	execer := execers.MakeSimExecerInterceptor(execers.NewSimExecer(), osexec.NewBoundedExecer(memory, memLT, *stat))
