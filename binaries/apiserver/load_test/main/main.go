...
when running locally, don't forget to open a tunnel to the apiserver:
ssh nest.smfc.twitter.com -N -L 50002:<apiserver:grpc port>
example:
ssh nest.smfc.twitter.com -N -L 50002:smf1-eza-15-sr1.prod.twitter.com:31223
the cas addr then in localhost:50002
*/
package main
...
import (
	...
	"github.com/twitter/scoot/binaries/apiserver/load_test"
...
func main() {
	...
	lt := load_test.MakeApiserverLoadTester(a)
...
func argParse() (*load_test.Args, bool, string, string, error) {
	...
	dataSizeMinFlag := flag.Int("min_data_size", 1,
		fmt.Sprintf("minimum data set size (in K bytes) for testing. Must be one of %s", load_test.TestDataSizesStr))
	dataSizeMaxFlag := flag.Int("max_data_size", 1,
		fmt.Sprintf("maximum data size (in K bytes) for testing. Must be one of %s", load_test.TestDataSizesStr))
	...
	startAsServerFlag := flag.Bool("start_as_http_server", false, "start an http service listening for get/put")
	serverNameFlag := flag.String("server_name", "localhost", "name for this load test server.")
...
*/
func validateArgs(a *load_test.Args, startAsServer bool, port string) bool {
	err := false
	...
	if ok := validateFileSize(a.DataSizeMin); !ok {
		log.Fatalf("min_file_size must be one of %s", load_test.TestDataSizesStr)
		err = true
	}

	if ok := validateFileSize(a.DataSizeMax); !ok {
		log.Fatalf("max_file_size must be one of %s", load_test.TestDataSizesStr)
		err = true
	}
...
func validateFileSize(size int) bool {
	ok := false
	for i := 0; i < len(load_test.TestDataSizes); i++ {
		if size == load_test.TestDataSizes[i] {
			ok = true
			break
		}
	}
