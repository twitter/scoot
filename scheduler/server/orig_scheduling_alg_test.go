...
// flaky test, commenting out since we are eliminating this scheduling algorithm
func Test_StatefulSchedulerRequestorCounts(t *testing.T) {
	...
	time.Sleep(500 * time.Millisecond) // let task threads run to complete
	...
	time.Sleep(500 * time.Millisecond) // let prior phase's tasks complete
	...
	time.Sleep(500 * time.Millisecond) // let prior phase's tasks' threads complete
	s.step()                           // start the next set of tasks
