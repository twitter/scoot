...
func (s *LocalSchedStrategy) Startup() (string, error) {
	...
	if err := s.cmds.Start(bin,
		"-thrift_addr", scheduler.DefaultSched_Thrift,
		"-http_addr", scheduler.DefaultSched_HTTP,
		"-log_level", s.workersCfg.LogLevel.String(),
		"-config", config); err != nil {
		return "", err
	}

	thriftPort, _ := strconv.Atoi(strings.Split(scheduler.DefaultSched_Thrift, ":")[1])
	httpPort, _ := strconv.Atoi(strings.Split(scheduler.DefaultSched_HTTP, ":")[1])
	...
	return scheduler.DefaultSched_Thrift, nil
}
