...
func (inv *Invoker) run(cmd *runner.Command, id runner.RunID, abortCh chan struct{}, updateCh chan runner.RunStatus) (r runner.RunStatus) {
	...
	case <-abortCh:
		...
		return runner.AbortStatus(id, runner.JobAndTaskID{JobID: cmd.JobID, TaskID: cmd.TaskID})
	case err := <-checkoutCh:
		if err != nil {
			return runner.ErrorStatus(id, err, runner.JobAndTaskID{JobID: cmd.JobID, TaskID: cmd.TaskID})
		}
	...
	if err != nil {
		return runner.ErrorStatus(id, fmt.Errorf("could not create stdout: %v", err), runner.JobAndTaskID{JobID: cmd.JobID, TaskID: cmd.TaskID})
	}
	defer stdout.Close()

	stderr, err := inv.output.Create(fmt.Sprintf("%s-stderr", id))
	if err != nil {
		return runner.ErrorStatus(id, fmt.Errorf("could not create stderr: %v", err), runner.JobAndTaskID{JobID: cmd.JobID, TaskID: cmd.TaskID})
	}
	defer stderr.Close()
	...
	if err != nil {
		return runner.ErrorStatus(id, fmt.Errorf("could not exec: %v", err), runner.JobAndTaskID{JobID: cmd.JobID, TaskID: cmd.TaskID})
	}
	...
	updateCh <- runner.RunningStatus(id, stdout.URI(), stderr.URI(), runner.JobAndTaskID{JobID: cmd.JobID, TaskID: cmd.TaskID})
	...
	case <-timeoutCh:
		p.Abort()
		return runner.TimeoutStatus(id, runner.JobAndTaskID{JobID: cmd.JobID, TaskID: cmd.TaskID})
	case st = <-processCh:
	}
	...
	case execer.COMPLETE:
		...
		if err != nil {
			return runner.ErrorStatus(id, fmt.Errorf("error staging ingestion dir: %v", err), runner.JobAndTaskID{JobID: cmd.JobID, TaskID: cmd.TaskID})
		}
		...
		if writer, err := os.Create(filepath.Join(tmp.Dir, stdoutName)); err != nil {
			return runner.ErrorStatus(id, fmt.Errorf("error staging ingestion for stdout: %v", err), runner.JobAndTaskID{JobID: cmd.JobID, TaskID: cmd.TaskID})
		...
		if writer, err := os.Create(filepath.Join(tmp.Dir, stderrName)); err != nil {
			return runner.ErrorStatus(id, fmt.Errorf("error staging ingestion for stderr: %v", err), runner.JobAndTaskID{JobID: cmd.JobID, TaskID: cmd.TaskID})
		...
		if err != nil {
			return runner.ErrorStatus(id, fmt.Errorf("error ingesting results: %v", err), runner.JobAndTaskID{JobID: cmd.JobID, TaskID: cmd.TaskID})
		}
		...
		status := runner.CompleteStatus(id, snapshotID, st.ExitCode, runner.JobAndTaskID{JobID: cmd.JobID, TaskID: cmd.TaskID})
		if cmd.SnapshotID != "" {
		...
		return status
	case execer.FAILED:
		return runner.ErrorStatus(id, fmt.Errorf("error execing: %v", st.Error), runner.JobAndTaskID{JobID: cmd.JobID, TaskID: cmd.TaskID})
	default:
		return runner.ErrorStatus(id, fmt.Errorf("unexpected exec state: %v", st.State), runner.JobAndTaskID{JobID: cmd.JobID, TaskID: cmd.TaskID})
	}
}
