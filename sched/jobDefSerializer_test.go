...
func Test_FixedJobDefinition(t *testing.T) {
	...
	origJobDef := JobDefinition{}
	origJobDef.Tasks = make(map[string]TaskDefinition)
	origJobDef.JobType = "jobTypeVal"
	...
	origJobDef.Tasks["task0"] = taskDefinition
	Print(origJobDef)


	serializeType := BinarySerialize

	// validate comparison logic
	fmt.Printf("validating comparison logic\n")
	...
	fmt.Printf("\nvalidating serialization logic\n")
	...
	if asByteArray, err := Serialize(&origJobDef, serializeType); err != nil {
		...
		fmt.Printf(fmt.Sprintf("serialized:%s\n",string(asByteArray[:])))
