...
def test_many():
  ...
  # Make the snapshots.
  ss_ids = make_snapshots()

  # Run the sleep script
  id = proto.run(argv=sleepDef.cmd, timeout_ns=rpc_timeout_ns, snapshot_id=ss_ids[sleepDef.snapshot_key])
  runs[id] = "sleep"
  ...
  for i in range(5):
      for j in range(10):
          try:
              id = proto.run(argv=lsDef.cmd, timeout_ns=rpc_timeout_ns, snapshot_id=ss_ids[lsDef.snapshot_key])
              runs[id] = "ls"
          except proto.ScootException as e:
      ...
      try:
          id = proto.run(argv=failDef.cmd, timeout_ns=rpc_timeout_ns, snapshot_id=ss_ids[failDef.snapshot_key])
      except proto.ScootException as e:
          if "Runner is busy" not in str(e):
              raise Exception("Run error, not resource limitation.{}".format(str(e)))
...
def make_snapshots():
  ...
  return {sleep_ss_key:sleep_ss_id, ls_ss_key:ls_ss_id, ls_fail_ss_key:fail_ls_ss_id}
